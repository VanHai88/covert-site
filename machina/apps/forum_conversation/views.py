...
from utils.comments import update_story
...
class TopicView(PermissionRequiredMixin, ListView):
    ...
    def get_topic(self):
        ...
        if not hasattr(self, 'topic'):
            self.topic = get_object_or_404(
                Topic.objects.select_related('forum').all(), uuid=self.kwargs['uuid'],
            )
        return self.topic
    ...
    def get_context_data(self, **kwargs):
        ...
        url = reverse('forum_conversation:topic', args=[forum.slug, forum.uuid, topic.slug, topic.uuid])
        context['current_url'] = 'https://%s%s' % (site, url)


        # Handles the case when a poll is associated to the topic
        context['first'] = topic.posts.first()
...
class BasePostFormView(FormView):
    ...
    def get_forum(self):
        ...
        uuid = self.kwargs.get(self.forum_uuid_url_kwarg, None)
        if not uuid:  # pragma: no cover
            # This should never happen
            return
        if not hasattr(self, '_forum'):
            self._forum = get_object_or_404(Forum, uuid=uuid)
        return self._forum
...
class PostFormView(SingleObjectMixin, BasePostFormView):
    ...
    forum_uuid_url_kwarg = 'forum_uuid'
        ...
        if not self.forum_post.approved:
            return reverse(
                ...
                kwargs={
                    ...
                    'uuid': self.forum_post.topic.forum.uuid,
                },
            )
        return reverse(
            ...
            kwargs={
                ...
                'forum_uuid': self.forum_post.topic.forum.uuid,
                'slug': self.forum_post.topic.slug,
                'uuid': self.forum_post.topic.uuid,
            },
        )


class TopicUpdateView(TopicFormView):
    ...
    def get(self, request, *args, **kwargs):
        """ Handles GET requests. """
        self.object = get_object_or_404(Topic, uuid=kwargs['uuid'])
        return super().get(request, *args, **kwargs)

    def post(self, request, *args, **kwargs):
        """ Handles POST requests. """
        #import pdb; pdb.set_trace()
        self.object = Topic.objects.get(uuid=request.POST.get('topic'))
        self.object.subject = request.POST.get('subject')
        self.object.content = request.POST.get('content')

        ftags = request.POST.get('ftags')
        if ftags:
            tags = json.loads(ftags)
        else:
            tags = []

        self.object.tags.add(*tags)
        self.object.tags.remove(*self.object.tags.exclude(id__in=tags))
        self.object.save()

        url = reverse('forum_conversation:topic', args=[self.object.forum.slug, self.object.forum.uuid, self.object.slug, self.object.uuid])
        if self.object.osubject != self.object.subject:
            update_story('DIS-' + self.object.uuid, url)
    ...
    def get_success_url(self):
        ...
        return reverse(
            'forum_conversation:topic', kwargs={
                'forum_slug': self.forum_post.topic.forum.slug,
                'forum_uuid': self.forum_post.topic.forum.pk,
                'slug': self.forum_post.topic.slug,
                'uuid': self.forum_post.topic.uuid,
            },
        )
    ...
    def get_success_url(self):
        ...
        return reverse(
            ...
            kwargs={
                ...
                'forum_uuid': self.object.topic.forum.pk,
