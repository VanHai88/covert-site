...
class PostForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['subject'].label = 'Topic'
        self.fields['subject'].widget.attrs['placeholder'] = _('Enter your topic')
...
class TopicForm(PostForm):
    ...
    def save(self, commit=True):
        """ Saves the instance. """
        ftags = self.data.get('ftags', '')

        if ftags:
            ftags = json.loads(ftags)
        ...
        if not self.instance.pk:
            ...
            self.instance.topic.tags.add(*ftags)
            self.instance.topic.tags.exclude(id__in=ftags).delete()
